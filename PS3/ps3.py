...
def get_word_score(word, n):
    ...
    c1 = sum(SCRABBLE_LETTER_VALUES[lower_word[i]] for i in range(wordlen))
    c2 = max(7 * wordlen - 3* (n-wordlen), 1)
    return c1*c2
...
def is_valid_word(word, hand, word_list):
    ...
    returns: boolean
    """
    star_position = word.find('*')
    temp_hand = hand.copy()
    lower_case_word = word.lower()

    #chek if word(include '*') in hand
    for i in lower_case_word:
        if i in temp_hand.keys():
            ...
            return False
    #check if word(without '*') in word_list
    if star_position == -1 :
        if lower_case_word in word_list:
            return True
        else:
            return False
    #check if word(with '*') in word_list
    else:
        for vowel in VOWELS:
            switch_star_to_vowel = lower_case_word[0:star_position] + vowel + lower_case_word[star_position+1:]
            if switch_star_to_vowel in word_list:
                return True
        return False
...
def substitute_hand(hand, letter):
    ...
    if letter in hand.keys():
        possible_letter = list(VOWELS+CONSONANTS)
        for i in hand.keys():
            if i == "*":
                pass
            else:
                possible_letter.remove(i)
        new_letter = random.choice(possible_letter)
...
def play_game(word_list, hand_size):
    ...
    * Returns the total score for the series of hands
    ...
    total_num_hand = int(input("Enter total number of hands: "))
    while total_num_hand > 0:
        ...
        if sub_letter == "no" or sub_letter =="No" or sub_letter =="NO":
            pass
        elif sub_letter == "yes" or sub_letter =="Yes" or sub_letter =="YES":
        ...
        if replay == "no" or replay =="No" or replay =="NO":
            ...
            Total_Score = play_hand(hand, word_list)
            print("----------")
        total_score_all_games = total_score_all_games + Total_Score
    print("----------")
    print("Total score over all hands:", total_score_all_games)
    return total_score_all_games
...
#improve: *nt , enter 'ant' directly 
# *nt can have multiple words
